...
/////////////////////////////////////////
#include "pubkey.h"
std::unique_ptr<ltcEthState> globalState;
/////////////////////////////////////////
...
valtype GetSenderAddress(const CTransaction& tx, const CCoinsViewCache* coinsView){
    ...
    for(size_t i = 0; i < txBit.vout.size(); i++){
        if(txBit.vout[i].scriptPubKey.HasOpCreate() || txBit.vout[i].scriptPubKey.HasOpCall()){
            if(receiveStack(txBit.vout[i].scriptPubKey)){
                ...
                ltcEthTransaction tx(createEthTX(params, i));
 ...
 ltcEthTransaction ByteCodeExec::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    ltcEthTransaction txEth;
    if (etp.receiveAddress == dev::Address()){
        txEth = ltcEthTransaction(txBit.vout[nOut].nValue, etp.gasPrice, (etp.gasLimit * etp.gasPrice), etp.code, dev::u256(0));
    }
    else{
        txEth = ltcEthTransaction(txBit.vout[nOut].nValue, etp.gasPrice, (etp.gasLimit * etp.gasPrice), etp.receiveAddress, etp.code, dev::u256(0));
    }
